...
Opus), not the process. Between steps the orchestrator stops at a **hard user checkpoint** — it
reports the step's outcome and waits for your explicit approval; no step starts without it. Each step's skill auto-loads when its turn comes (you can also invoke it
...
- **Hard user checkpoint between steps.** The orchestrator stops after every step and waits for the
  user's explicit approval before the next one — steps never chain automatically.
