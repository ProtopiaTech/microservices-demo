...
5. **Commit the artifact.** Once the reviewer passes with no open items, the orchestrator makes
   ...
   **unit's branch** per `docs/commit-conventions.md` — **before** the checkpoint. Stage by path
...
6. **Checkpoint — stop.** Present the plan, then **end the turn and wait for the user's explicit
   approval** — no code is written before it. `work-execute` (Step 4, walks the final TODO
   checklist) starts only after they approve — never in the same turn.
