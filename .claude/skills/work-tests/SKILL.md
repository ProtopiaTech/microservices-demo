...
4. **Commit the artifact.** Once the reviewer passes, the orchestrator makes **one atomic
   ...
   `docs/commit-conventions.md` — **before** the checkpoint. Stage by path
...
5. **Checkpoint — stop.** Present the behaviour spec, then **end the turn and wait for the user's
   explicit approval** — this is the cheapest place to catch a misunderstanding. `work-plan` (Step 3,
   consumes `1-research.md` + `2-tests.md`) starts only after they approve — never in the same turn.
